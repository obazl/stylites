...
https://en.wikipedia.org/wiki/Simeon_Stylites[Simeon Stylites] was not a tree-sitter, but he was the original column-sitter.
...
* link:examples/imp[examples/imp] contains an implementation of the https://softwarefoundations.cis.upenn.edu/lf-current/Imp.html[imp] language (grammar from https://siraben.dev/2022/03/01/tree-sitter.html[How to write a tree-sitter grammar in an afternoon]). Builds the parser as a `cc_library` and includes a `cc_test` target that uses https://github.com/ThrowTheSwitch/Unity[Unity].
...
- https://tree-sitter.github.io[Documentation]
- https://crates.io/crates/tree-sitter[Rust Tree-sitter]
- https://www.npmjs.com/package/web-tree-sitter[Web Tree-sitter]
- https://github.com/tree-sitter/tree-sitter/tree/master/cli[Command-line interface]
