...
pub fn generate_node_binding_files(repo_path: &Path, language_name: &str) -> Result<()> {
    println!("generate_node_binding_files: {}", repo_path.display());

    let bindings_dir = repo_path.join("bindings");
    println!("bindings_dir: {}", bindings_dir.display());
...
pub fn generate_rust_binding_files(repo_path: &Path, language_name: &str) -> Result<()> {
    println!("generate_rust_binding_files: {}", repo_path.display());
    let bindings_dir = repo_path.join("bindings");
