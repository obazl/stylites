...
fn run() -> Result<()> {
    ...
    let matches = App::new("corpus")
        .about("Converts corpus tests to Unity tests.")
        ...
        .arg(&debug_arg)
        .arg(
            Arg::with_name("corpus-dir")
                .help("Path to directory of corpus test files")
                .long("corpus")
                .takes_value(true),
        )
        .arg(
            Arg::with_name("case-path")
                .help("Path to one corpus test file")
                .long("case")
                .takes_value(true),
        )
        .get_matches();
